...
import { dlsiteFemaleDoujinAdapter, fetchDlsiteProductDetailForDebug, type DlsiteProductDebugFloor } from "./adapters/dlsite/dlsiteFemaleDoujinAdapter";
...
function buildManualFetchOptions(query: Record<string, unknown>, isEmulator: boolean) {
  return {
    ...
    skipFreshHours: parseIntegerQuery(query.skipFreshHours, { min: 0, max: 24 * 30 }),
  };
}
...
function inferContentTypeForDebug(floor: "girls" | "bl", contentTypeIds?: string[]): ProductContentType {
  if (contentTypeIds?.some((value) => value.toLowerCase() === "dlsite:bl")) return "bl";
  if (contentTypeIds?.some((value) => value.toLowerCase() === "dlsite:tl")) return "tl";
...
function errorToDebugPayload(error: unknown): { message: string; name?: string; cause?: string } {
...
export const fetchDlsiteProductDebug = onRequest(
  ...
  async (req, res): Promise<void> => {
    ...
    const sourceProductId = normalizeDlsiteDebugProductId(req.query.productId ?? req.query.sourceProductId);
    if (!sourceProductId) {
      res.status(400).json({ ok: false, message: "productId must be like RJ01234567" });
      return;
    }
    ...
    try {
      const debugResult = await fetchDlsiteProductDetailForDebug({ sourceProductId, floor });
      const rawForType = debugResult.rawProductDetail as { contentTypeIds?: string[]; images?: unknown[] };
      const contentType = inferContentTypeForDebug(debugResult.selectedFloor, rawForType.contentTypeIds);
      const target = buildDebugTarget(contentType);
      const product = dlsiteFemaleDoujinAdapter.normalizeProduct(debugResult.rawProductDetail, target);
      ...
      if (saveHtml) {
        ...
        await db.collection("debugDlsiteProductHtml").doc(debugHtmlDocId).set({
          debugHtmlDocId,
          sourceProductId,
      ...
      logger.error("fetchDlsiteProductDebug failed", { sourceProductId, floor, ...errorPayload });
      res.status(500).json({ ok: false, productId: sourceProductId, floor, ...errorPayload });
    }
  },
);
