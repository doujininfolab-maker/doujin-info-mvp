...
export default async function RankingPage({ params }: PageProps) {
  ...
  const products = await getLatestRankingProducts({ platform: segment.platform, audience: segment.audience, category: segment.category, limitCount: 30, rankingType: "daily" });
  return (
    <div className="listPage">
      <SegmentNav segment={segment} />
      <section className="contentSection">
        <SectionHeader title="人気ランキング" description={`${segment.label}の人気作品`} icon="♕" />
        <ProductGrid products={fillProducts(products, 12)} showRank variant="grid" />
      </section>
    </div>
  );
}
